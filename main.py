...
def get_args_parser():
    ...
    # Dataset parameters
    parser.add_argument('--data-path', default='./image_data', type=str,
                        help='dataset path')
    parser.add_argument('--data-set', default='IMNET', choices=['CIFAR', 'IMNET', 'INAT', 'INAT19','TXT'],
    ...
    parser.add_argument('--val-txt', default='', type=str,
                    help='Path to val list txt for data-set=TXT (optional)')
    parser.add_argument('--label-txt', default='label.txt', type=str,
                    help='Path to label name txt for data-set=TXT (used to infer num classes)')
    ...
    parser.add_argument('--pred-label-out', default='pred_labels.txt', type=str,
                    help='Output file: <img_path> <label1,label2,...>')
    parser.add_argument('--thr', default=0.5, type=float,
                    help='Threshold for multilabel prediction (sigmoid>=thr => 1)')
...
def main(args):
    ...
    mixup_fn = None
    mixup_active = (args.data_set != 'TXT') and (args.mixup > 0 or args.cutmix > 0. or args.cutmix_minmax is not None)
    if mixup_active:
        mixup_fn = Mixup(
    ...
    model = create_model(
        ...
        distillation=(args.distillation_type != 'none' and args.data_set != 'TXT'),
    ...
    if args.data_set == 'TXT':
        args.distillation_type = 'none'
        pos_weight = None
        if args.pos_weight:
            pw = [float(x) for x in args.pos_weight.replace(' ', '').split(',') if x != '']
    ...
    teacher_model = None
    if args.distillation_type != 'none' and args.data_set != 'TXT':
    ...
    if args.eval:
        if args.data_set == 'TXT' and getattr(args, "predict_only", False):
            ...
            pred_info = predict_txt_images(
                ...
                out_vec_path=str(Path(args.output_dir) / args.pred_out) if args.output_dir else args.pred_out,
                out_label_path=str(Path(args.output_dir) / args.pred_label_out) if args.output_dir else args.pred_label_out,
                thr=args.thr
            )
            if utils.is_main_process():
                print(f"[predict_only] saved vec to: {pred_info['out_vec']}")
                print(f"[predict_only] saved labels to: {pred_info['out_labels']}")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir and (epoch + 1) % args.save_freq == 0:
            checkpoint_paths = [output_dir / f'checkpoint_epoch{epoch}.pth']
        ...
        if epoch % 4 == 3:
            test_stats = evaluate(data_loader_val, model, device)
            print(f"mAP: {test_stats['mAP']*100:.2f}% | micro-F1: {test_stats['micro_f1']*100:.2f}% (thr={test_stats['best_thr']:.2f})")
            max_accuracy = max(max_accuracy, test_stats["mAP"]*100)
